...
export default async function BookOverview({ lang, intl }: BookOverviewProps) {
  ...
  return <div className="library-overview p-4">
    <div className="library-overview-section"><BookRowCarousel books={books.slice(0, 12)} lang={lang} intl={intl} header={<h2 className="flex items-center text-base md:text-lg"><BookPlusIcon size={28} className="mr-2" />{intl.libraries.recentlyAdded as string}</h2>} /></div>
    {recentlyRead.length > 0 && <div className="library-overview-section"><BookRowCarousel books={recentlyRead} lang={lang} intl={intl} header={<h2 className="flex items-center text-base md:text-lg"><BookCheckIcon size={28} className="mr-2" />{intl.libraries.recentlyRead as string}</h2>} /></div>}
  </div>;
}

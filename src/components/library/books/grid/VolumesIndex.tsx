...
export default async function VolumesIndex({ lang, intl }: { lang: Locale; intl: Dictionary }) {
  ...
  return <><div className="mb-4 flex items-center"><h2 className="flex items-center text-base md:text-lg"><LibraryBigIcon size={28} className="mr-2" />{intl.libraries.books as string}</h2></div><BookGrid books={books} lang={lang} intl={intl} progressById={progressById} /></>;
}

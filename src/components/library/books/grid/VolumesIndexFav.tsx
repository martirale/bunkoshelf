...
export default async function VolumesIndexFav({ lang, intl, page = 1 }: { lang: Locale; intl: Dictionary; page?: number }) {
  ...
  return <>
    <h2 className="mb-4 flex items-center"><BookCopyIcon size={28} className="mr-2" />{books.favoriteBooks}</h2>
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
      {favorites.items.map((volume) => <MangaCard key={volume.id} title={volume.metadata.title} href={`/${lang}/books/volume/${volume.slug}`} isSeries={false} isOneshot={false} onGoing={false} onPause={false} volumeCount={null} cover={getBookCoverUrl(volume.slug, volume.metadata.coverPath)} progressRatio={progressById[volume.id]?.progression ?? null} isDragging={false} seriesSlug={null} offlineVolumeId={volume.id} intl={intl} className="font-roboto text-base font-bold leading-5 2xl:text-lg" />)}
    </div>

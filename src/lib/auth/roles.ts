import type { Role, Permission, Session } from "@/lib/types";
...
const ROLE_HIERARCHY: Role[] = [ROLES.GUEST, ROLES.MEMBER, ROLES.ADMIN];
...
export function isSuperAdmin(user: Session | null): boolean {
...
export function requireRole(user: Session | null, role: Role): boolean {
  if (!user) return false;
  if (isSuperAdmin(user)) return true;
